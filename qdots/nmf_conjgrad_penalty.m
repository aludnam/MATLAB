...
options_h(7)=1;
options_h(9)=1;                   %to check gradient
...
while and(dd>peval.ddterm, ii<peval.maxiter)
    if ~isempty(h)
        [rHkte, options_h, flog_h, pointlog_h] = conjgrad('ddivHexp_penalty', reshape(log(h),1,m*kh), options_h, 'gradddivHexp_penalty', v, w, w_fix, h_fix, peval);
