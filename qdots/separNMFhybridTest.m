...
% % %                     hinit = [f*blinkmat./repmat(mean(blinkmat,2),1,size(blinkmat,2)); bg(mm)*sumw(ncomp)*ones(1, p.Nt)];             %random weights will be assigned to firts two and bg fixed
                    ...
                    [c,w{mm},h{mm}, X1,X2, dhr, minXr, miXvalr, xcovpeakr, meanabsxcr,mhdr, p]=nmf_testconvD_normalWhybridTest(double(dveccr),ncomp,1,winit,hinit, [3], [3],p);
                    
                    
                    hinit = [f*blinkmat./repmat(mean(blinkmat,2),1,size(blinkmat,2)); bg(mm)*sumw(ncomp)*ones(1, p.Nt)];             %random weights will be assigned to firts two and bg fixed
                    [c,w{mm},h{mm}, X1,X2, dht, minXt, miXvalt, xcovpeakt, meanabsxct,mhdt, p]=nmf_testconvD_normalWhybridTest(double(dveccr),ncomp,1,winit,hinit, [3], [3],p);
                    qqq=[];
                else
