...
% % %                     hinit = [f*blinkmat./repmat(mean(blinkmat,2),1,size(blinkmat,2)); bg(mm)*sumw(ncomp)*ones(1, p.Nt)];             %random weights will be assigned to firts two and bg fixed
                    ...
                    [c,w{mm},h{mm}, X1,X2, dhr, minXr, miXvalr, p]=nmf_testconvD_normalWhybridTest(double(dveccr),ncomp,1,winit,hinit, [3], [3],p);
                    
                    
                    hinit = [f*blinkmat./repmat(mean(blinkmat,2),1,size(blinkmat,2)); bg(mm)*sumw(ncomp)*ones(1, p.Nt)];             %random weights will be assigned to firts two and bg fixed
                    [c,w{mm},h{mm}, X1,X2, dht, minXt, miXvalt, p]=nmf_testconvD_normalWhybridTest(double(dveccr),ncomp,1,winit,hinit, [3], [3],p);
                    qqq=[];
                else
        ...
        if savethis == 1
            ...
            %             save (p.namedir)
            save ([namedir '_separMulti'],'p', 'X1','X2','dhr','minXr', 'miXvalr','dht','minXt', 'miXvalt')
