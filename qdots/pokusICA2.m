...
for ii=1:Nt
    dpix(:,:,ii) = pixelize(d, blinkmat(:,ii), sizevec, nx, ny ,[], 0);
    dpixc(:,:,ii) = imresize(conv2(dpix(:,:,ii),f,'same'),rs);
end

% dvec = squeeze(reshape(dpixc, (nx+sf(1))*(ny+sf(2)), 1, Nt));
dvec = squeeze(reshape(dpixc, (nx)*(ny)*rs^2, 1, Nt));

toc(tstart)
cd ~/project/MATLAB/FastICA_25/
numOfIC = N;
[icasig, A, W] = FASTICA (dvec', 'numOfIC', numOfIC, 'g', 'tanh');
