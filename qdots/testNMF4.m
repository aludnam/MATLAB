...
% [X1, X2] = meshgrid(14.8:0.05:15.0, 14.8:0.05:15.0);
% [X1, X2] = meshgrid(14.8:0.05:16.5, 14.8:0.05:16.5);
[X1, X2] = meshgrid(14.95:0.05:15.2, 14.95:0.05:15.25);
...
% % %             h(h_dovec,:)=h(h_dovec,:) + etha*(y1(h_dovec,:) -1);
            ...
            [xc, lags] = xcov(h(1,:), h(2,:), 'none');
            xcix=find(lags==0);
            xcp=xc(xcix); %value in zero 
            xc_m=[xc(1:xcix-1),xc(xcix+1:end)];        
            xcp_m=max(xc_m);
            ratxc=abs(xcp)/abs(xcp_m); 
            if ratxc>tolratxc
                h(2,:)=hinit(2,:); % reset one of teh components
            end
            %             %%%testing
            % %             dh(ixmat,iih)=d(k);
            % %             iih=iih+1;
            ...
            switch ih
                case 10              
                    ...
                    ratioxc(ixmat,jxmat,zxmat)=ratxc;
                    zxmat = zxmat+1;
                case 100
                    dh(ixmat,jxmat,zxmat)=ddivergence(v, w*h);
                    htr(zxmat,:,:) = h;
