...
while and(dd>peval.ddterm, ii<peval.maxiter)
    if ~isempty(h)
        [rHkte, options_h, flog_h, pointlog_h] = conjgrad('ddivHexp_sparsMultipl', reshape(log(h),1,m*kh), options_h, 'gradddivHexp_sparsMultipl', v, w, w_fix, h_fix, peval);
