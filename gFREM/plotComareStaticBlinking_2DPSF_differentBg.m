...
bgvec = [0 100 1000];
vard = [vard_bg0, vard_bg100, vard_bg1000];
vardintout = [vardintout_bg0, vardintout_bg100, vardintout_bg1000];
clear('l')
...
for ii=m:length(bgvec)
    ...
    l{jj}=['Background ' num2str(bgvec(ii)) ' photons'];
    jj=jj+1;
end
legend(l)
% vline2(raylim,'k--')
...
if savethis 
    name = ['images/ComarisonStaticVsBlinking_PSF2D_FREM_Lambda' num2str(int1_multi{1}/2) '_differentBg'];    
    SaveImageFULL(name)
end
