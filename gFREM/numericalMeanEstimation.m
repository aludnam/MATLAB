...
dl1=dlbgMat(:,:,1);
dl2=dlbgMat(:,:,2);
masksPo = abs(sPo)>precMask; % to avoid division of small numberss...
% mask=and(dl1.^2>precMask,masksPo);
tmp = dl1.^2.*It11;
mask=tmp>precMask;
Ie11=sum(tmp(mask)./sPo(mask),1)';
% mask=and(dl2.^2>precMask,masksPo);
tmp = dl2.^2.*It22;
mask=tmp>precMask;
Ie22=sum(dl2(mask).^2.*It22(mask)./sPo(mask),1)';
% mask=and(dl1.*dl2>precMask,masksPo);
tmp=dl1.*dl2.*It12;
mask=tmp>precMask;
