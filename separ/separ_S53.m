        ...
        for qq=1 : length(avg_num)
            ...
            for mm=1:niter
                ...
                if sum(sep_how == 'n')>0 %NMF
                    ...
                    [c,w{mm},h{mm}, X1,X2, dh, minX, miXval, mhd, htr,htrace, p]=nmf_S53(double(dveccr),ncomp,1,winit,hinit, [3], [3],p);
                    res=struct('c',c,'w',w,'h',h,'X1',X1,'X2',X2,'dh',dh,'minX',minX,'miXval',miXval, 'mhd',mhd, 'htr',htr, 'htrace',htrace,'p',p);
